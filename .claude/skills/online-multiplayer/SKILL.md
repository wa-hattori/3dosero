...
## Firestoreドキュメントのスキーマを変更する時の指針

このプロジェクトにはCloud Functions等のサーバー側処理が無い（[CLAUDE.md](../../../CLAUDE.md)の「サーバーコードを自前で書かない」方針）ため、Firestoreドキュメントの形を変える際に、既存データを一括変換するバッチ処理を自動では持てない。**リリース後（実プレイヤーのデータが既に存在する状態）でスキーマを変更する場合は、以下を検討する。**

- **読み取り側は常に旧形状に対して防御的にする。** 新フィールドが無い・別の形をしている古いドキュメントを読んでも例外を投げず、妥当な初期値にフォールバックする（実例: [ranked-matchmaking](../ranked-matchmaking/SKILL.md)で`score`/`gamesPlayed`のフラットな2フィールドを`ratings`（盤面サイズをキーとするマップ）へ移行した際、`data.ratings?.[boardSize] ?? { score: DEFAULT_SCORE, gamesPlayed: 0 }`という形で「該当エントリが無ければ初期値扱い」にした）。これにより、移行前の古いドキュメントを読んでもアプリがクラッシュしない状態を保てる。
- **ただし「フォールバックで動く」ことと「過去の実績が保持される」ことは別問題。** 上記の例で言えば、旧`score`フィールドに蓄積されていた値は新しい`ratings`マップには自動的には引き継がれない（アプリからはDEFAULT_SCOREからの再スタートに見える）。実プレイヤーのデータが存在する状態で本当に実績を引き継ぎたい場合は、**一度きりの移行スクリプトが必要になる**。このプロジェクトの通常のクライアントコード（`src/net/`、Firebase Web SDK・匿名認証・Firestoreセキュリティルールの制約下で動く）では、他人のドキュメントを一括で書き換えるような操作はそもそも権限的にできない設計になっているため、移行スクリプトは`firebase-admin`（サービスアカウント鍵によるセキュリティルールを迂回した管理者権限アクセス）を使う別立ての一回限りのNode.jsスクリプトとして書き、実行後は削除する（恒久的なコードとして`src/`配下には置かない）。サービスアカウント鍵の発行・保管はユーザー側の判断が必要なため、移行が必要になった時点で改めて相談する。
- **可能なら移行を避けられる設計を優先する。** 例えば新フィールドを追加するだけ（既存フィールドは変えない）であれば移行は不要。既存フィールドの形そのものを変える場合でも、「新旧両方の形を読める」防御的な読み取りコードをまず入れてから、書き込み側を新形状に切り替える、という順序にすると、移行スクリプトなしで新規プレイヤーは自然に新形状に、既存プレイヤーは次に書き込みが発生したタイミングで段階的に新形状へ移行できる場合がある（[ranked-matchmaking](../ranked-matchmaking/SKILL.md)の`ratings`移行はこのパターンを採用しており、旧`score`の実績を引き継がない代わりに移行スクリプト無しで済ませている）。
