...
9. **TestFlightで内部テスターにビルドを配信する。** App Store Connect → 対象アプリ → **TestFlight**タブ → 左サイドバー「テスター」欄の **「内部テスト」**（英語UIでの"Internal Testing"。トップレベルの別タブではなく、TestFlightタブ内の左サイドバーに小さく表示される）→ グループを作成 → テスター（自分のApple ID）を追加 → そのグループにビルドを紐づける。詰まりやすい点は下記「エッジケース・注意点」11〜12を参照。
...
15. **`bootstrap_signing_identity`レーンが成功と表示されるのに、アップロードされるはずの`.p12`アーティファクトが見つからない（"No files were found with the provided path"）。** Fastfile内で素の`File.expand_path("相対パス")`（`Dir.pwd`基準）を使うと、実際には`ios-app/`ではなく`ios-app/fastlane/`配下に書き出されてしまう（fastlaneの各アクションが内部で解決する「プロジェクトルート」と、Rubyプロセス自身の`Dir.pwd`は別物であり、素のRubyコード〈`File`/`Tempfile`等〉は後者を基準にする）。**この不具合の再現時点で、`get_certificates`自体は実行済み＝証明書を1つ消費してしまっている**（エクスポート先を間違えただけで、証明書の発行自体は成功しているため）。対処: パス解決に`__dir__`（Fastfileファイル自身の場所）を明示的な基準にする（`File.expand_path("../exported-signing-identity.p12", __dir__)`）。再実行する前に、直前の（回収できなかった）証明書をCertificatesページで失効させ、証明書発行上限を圧迫しないようにすること。
16. **App Store Connectで審査提出時に「13インチのiPadディスプレイのスクリーンショットをアップロードする必要があります」と言われる。** `npx cap add ios`直後の`App.xcodeproj`は`TARGETED_DEVICE_FAMILY = "1,2"`（iPhone + iPad = Universal）がデフォルトで、iPadにも対応しているとApple側に登録されるため。このアプリはiPadでの動作確認をしておらず、対応する設計にもなっていないため、`TARGETED_DEVICE_FAMILY`を`1`（iPhoneのみ）に変更してiPad対応自体を外す（`ios-app/ios/App/App.xcodeproj/project.pbxproj`のDebug/Release両方）。iPad対応を維持したい場合は、この設定を変えず、別途iPadでの動作確認とスクリーンショット撮影が必要になる。この設定変更は次のビルド以降にしか反映されない（既にアップロード済みのビルドはUniversalのまま）ため、審査提出に使うビルドを作り直す必要がある。
17. **App Store Connectで審査提出時に「Xcodeに`com.apple.developer.game-center`キーを追加する必要があります」と言われる。** このアプリはGame Centerを使わない設計（[App ID登録時のCapabilities](#app-id登録時のcapabilities)参照）だが、[Certificates, Identifiers & Profiles → Identifiers](https://developer.apple.com/account/resources/identifiers/list)上のApp ID（`com.wahattori.threedosero`）側でGame Center Capabilityが有効になっていると、ローカルのXcodeプロジェクト（Game Center entitlementを持たない）との不一致でこのエラーになる。**Xcode側にentitlementを追加するのではなく、Apple Developer Portal側でApp IDのGame Center Capabilityのチェックを外すのが正しい対処**（実装を増やして不一致を解消するのではなく、そもそも使わない機能の登録を消す）。
