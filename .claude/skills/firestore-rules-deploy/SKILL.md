...
4. **本番反映は、実行前にユーザーに一声かける。** ドライランと違い、これは**即座に・段階を踏まずに**本番のセキュリティルールを書き換える（[static-deploy](../static-deploy/SKILL.md)のアプリコード配信のように、バージョンタグpush→GitHub Actions実行という確認の挟まる仕組みが無い）。実データがまだ存在しない開発初期段階でも、「このタイミングで書き換えてよいか」は都度確認する（一度の許可が以降のセッション・以降の変更にまで及ぶわけではない）。ユーザーから明示的に「今後はルール変更のたびに確認なしでデプロイしてよい」と durable な指示があった場合はこの限りではない。
   ```
   npx firebase deploy --only firestore:rules
   ```
