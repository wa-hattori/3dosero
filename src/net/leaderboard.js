 ...
 * 不要（Firestoreが単一フィールドインデックスを自動的に用意する）。
 * @returns {Promise<Array<{ name: string, score: number, gamesPlayed: number }>>}
...
export const fetchLeaderboard = async () => {
  ...
  return snapshot.docs.map((docSnapshot) => {
    ...
    return { name: data.name, score: data.score, gamesPlayed: data.gamesPlayed };
  });
};
